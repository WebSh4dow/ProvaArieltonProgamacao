...
import com.prova.agenda.view.EditarContato;
...
public class PrincipalController implements Initializable {
    ...
    private void preencherTabela() {
        List<Contato> contatos = contatoService.consutarTodosContatos();
        if (isFiltro) {
            contatos = contatoService.consutarContatosFiltrados(this.filtro);
        }
        if (contatos.size() > 0) {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        btn_sair.setOnMouseClicked((action) -> {
            HelloApplication.getPrincipal().close();
        });
